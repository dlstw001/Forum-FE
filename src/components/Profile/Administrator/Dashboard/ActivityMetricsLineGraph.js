...
const ActivityMetricsLineGraph = ({ data }) => {
  var dateArray = [];

  data.data && data.data.date.map((item) => dateArray.push(dateFormat(item)));
...
export default inject(() => ({}))(observer(ActivityMetricsLineGraph));
