  ...
  return (
    <div className="sidebar-items-row">
      <div className="sidebar-items">
        <Link to={`${ROUTES.CASES}/${data.slug}`}>{data.title}</Link>
      </div>
    </div>
  );
};

const RelatedTopicsList = ({ tags, caseStore }) => {
  const [relatedByTag, setRelatedByTag] = React.useState({ data: [] });

  const getData = React.useCallback(() => {
    if (tags.length) {
      caseStore.relatedByTagCases({ page: 1, limit: 10, tags: tags }).then((res) => setRelatedByTag(res));
    }
  }, [caseStore, tags]);
...
export default inject(({ caseStore }) => ({ caseStore }))(observer(RelatedTopicsList));
