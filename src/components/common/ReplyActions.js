...
const ActionBar = ({
  ...
  const handleLikeClick = async () => {
    if (data.liked) {
      const resRemoveLike = await store.removeLike(data.post._id, data._id);
      setNoLikes(resRemoveLike.item.likes.length);
      // data.noLikes = data.noLikes - 1;
      data.liked = false;
    } else {
      const resLike = await store.like(data.post._id, data._id);
  ...
  const copyToClipboard = () => {
    const { replyNo } = data;
    const url = `${window.location.host}${urlGenerator(post, replyNo)}`;
    const httpUrl =
    ...
    <>
      <div className="flex flex-wrap items-center response-action">
        {handleForm && !post.deleted && (!post.archived || !post.closed) && !data.deleted && userStore.user && (
          <button
            className="btn btn-response-action reply"
            onClick={() => handleForm({ data, replyTo: data._id, mode: 'reply' })}
          >
            Reply
          </button>
        )}
        ...
        {!post.deleted &&
          (userStore.IS_ADMIN_OR_MODERATOR || data?.creator?._id === userStore.user?._id || userStore.IS_LEADER) &&
          !data.deleted && (
            <>
        ...
        {!isMessage &&
          !post.deleted &&
          (post?.creator?._id === userStore.user?._id || userStore.IS_ADMIN_OR_MODERATOR) &&
          !data.deleted && (
            <button
              className={cx('btn btn-response-action ml-4', { 'text-primary-dark': solution })}
              onClick={onSolution}
              data-cy={`solution_reply_btn_${data._id}`}
            >
              <i className="mr-1 md-16 material-icons">check_box</i>
              Solution
            </button>
          )}
        {!isMessage &&
...
export default inject(({ userStore, replyStore, postStore }) => ({
  userStore,
  replyStore,
  postStore,
}))(observer(ActionBar));
